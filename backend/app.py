...
from gemini_utils import generate_plant_details
...
irrelevant_model.load_state_dict(torch.load(os.path.join(os.path.dirname(__file__), "models", "mobilenet_irrelevent.pt"), map_location=device))
...
def predict_classification(image_path):
    ...
    main = predictions[0]["plant"] if predictions else "Unknown"
...
def api_upload():
    try:
        ...
        upload_dir = os.path.join(os.path.dirname(__file__), "static", "uploaded_images")
        ...
        gemini_details = None
        try:
            gemini_details = generate_plant_details(plant)
        except Exception:
            gemini_details = None
...
def send_image(filename):
    return send_from_directory(os.path.join(os.path.dirname(__file__), "static", "uploaded_images"), filename)
